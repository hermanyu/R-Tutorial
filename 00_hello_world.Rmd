...
1.  Data types
2.  Operations
3.  Variable and function assignment
4.  Control flow
5.  Loops
...
We can also take *absolute values* of numbers as well. This is done via the `norm()` function in R:

```{r}
# norm aka absolute value of the number -3 
norm(-3, type="2")

# norm of the number 3
norm(3, type="2")

# norm of the complex number 3+4i
norm(3+4i, type="2")
```
...
### 2.3 Variable and Function Assignment
...
We can also assign functions to manipulate data directly. This is done using the following syntax:

```{r}
# define a function called f which takes 2 inputs and does something
f <- function(input_1, input_2){
  
  # the block of text inside the braces is what the function actually does
  my_sum <- input_1+input_2
  
  # we can also return some output value, if we so desire
  return(my_sum)
}
```

We can then call our function using standard function notation:
